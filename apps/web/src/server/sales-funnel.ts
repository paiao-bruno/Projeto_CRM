...
import type { ServerAuthUser } from "./auth";
...
class SalesFunnelService {
  ...
  async getBoard(user: ServerAuthUser, filters: DealFilters = {}) {
  ...
  async listDeals(user: ServerAuthUser, query: Record<string, any>) {
  ...
  async getDeal(user: ServerAuthUser, dealId: string) {
    ...
    if (!deal) throw new HttpError(404, "Oportunidade não encontrada.");
    return deal;
  }

  async createDeal(user: ServerAuthUser, dto: Record<string, any>) {
    const pipeline = await this.ensureDefaultPipeline(user.tenantId);
    const stage =
      pipeline.stages.find((item) => item.code === dto.stageCode) ?? pipeline.stages[0];
    if (!stage) throw new HttpError(400, "Etapa inicial do funil não encontrada.");
  ...
  async updateDeal(user: ServerAuthUser, dealId: string, dto: Record<string, any>) {
  ...
  async moveDeal(user: ServerAuthUser, dealId: string, dto: Record<string, any>) {
    ...
    if (!targetStage) throw new HttpError(400, "Etapa de destino inválida.");
  ...
  async markLost(user: ServerAuthUser, dealId: string, dto: Record<string, any>) {
    const deal = await this.requireDeal(user.tenantId, dealId);
    this.assertEditable(deal);
    ...
    if (!lossReason) throw new HttpError(400, "Informe o motivo da perda.");
  ...
  async archiveDeal(user: ServerAuthUser, dealId: string) {
    const deal = await this.requireDeal(user.tenantId, dealId);
  ...
  async restoreDeal(user: ServerAuthUser, dealId: string) {
    const deal = await this.requireDeal(user.tenantId, dealId);
    if (!deal.archivedAt) return this.getDeal(user, dealId);
  ...
  private assertEditable(deal: { archivedAt: Date | null; status: DealStatus }) {
    if (deal.archivedAt) {
      throw new HttpError(403, "Oportunidade arquivada não pode ser alterada.");
    }
    if (deal.status === DealStatus.LOST) {
      throw new HttpError(403, "Oportunidade perdida não pode ser alterada.");
    }
  }
...
class SalesFunnelMetricsService {
  private get prisma() {
    return getPrisma();
  }

  async getMetrics(tenantId: string, query: Record<string, any>) {
...
export function createSalesFunnelService() {
  return new SalesFunnelService();
}

export function createSalesFunnelMetricsService() {
  return new SalesFunnelMetricsService();
}
